...
def start(config: dict, test_data: list[dict]):
    ...
    for data in test_data:
        ...
        filtered_test_columns = [
            col for col in transaction_selected if col in test_df.columns
        ]
        ...
        partner_df = cleaner.clean_text(
            partner_df, columns=text_columns.get("partner", [])
        )
        ...
        partner_df_text_vectorize_path = partner_df_text_vectorize.get(
            "save_load_path", []
        )
        ...
        partner_df_text_similarity_columns = partner_df_text_similarity.get(
            "columns", []
        )
        partner_df_text_similarity_methods = partner_df_text_similarity.get(
            "methods", []
        )
        ...
        def predict_lifnr_kunnr(
            ...
            top_5_indices = y_pred.argsort()[-5:][::-1]
